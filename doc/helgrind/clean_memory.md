...
treated as if it has just been allocated to the calling thread.
...
- **External Synchronization**: Any other thread attempting to access this range
  must synchronize with the calling thread first; otherwise, Helgrind will
  report a race condition.

# Use Case

This is primarily required for custom memory allocators or memory pools. When
memory is recycled (freed by one thread and subsequently reallocated to
