...
This function wraps the `VALGRIND_MAP_IP_TO_SRCLOC` macro. It queries the
debug information available to Valgrind for the specified instruction
address and writes the result into the provided buffer.
...
Rust's release profile strips debug info by default. To use this request in a release build, 
debug info must be preserved in `Cargo.toml`:
```toml
[profile.release]
