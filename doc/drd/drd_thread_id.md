...
The returned ID matches the thread IDs reported in DRD data race reports and
trace messages, making it useful for correlating log output with internal state.
...
- **Stable:** IDs are **never recycled**. When a thread exits, its ID is not
  reused. This ensures the ID remains consistent throughout the execution log.
