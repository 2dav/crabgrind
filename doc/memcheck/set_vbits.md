...
- `vbits` - Buffer containing the V-bit data. The length determines the number
  of bytes modified.

# Warning

Use with extreme caution. Incorrect V-bit data can silence legitimate errors or
cause Memcheck to report false positives. It is generally recommended to only
set V-bits that were previously retrieved via [`vbits`](vbits).
